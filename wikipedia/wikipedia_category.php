...
$ind = 0;
while (true) {
	$tweets = query("SELECT DISTINCT(wikiID) FROM conceito c WHERE sucesso = 1 AND wikiID IS NOT NULL AND NOT EXISTS (SELECT w.resource FROM wikipedia_category w WHERE w.resource = c.resource) LIMIT 10");
