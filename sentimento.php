...
$tweets = query("SELECT * FROM tweets WHERE situacao = 'S'");
...
foreach (getRows($tweets) as $key => $value) {
	...
	try {
        ...
        if (checkEmoji2($texto)) {
            ...
            $sa->analyse(implode(" ", $emoticons));
            debug("-----------");
            debug($emoticons);
            debug($sa->return_sentiment_rating());
        }
